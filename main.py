...
print(Fore.GREEN + Style.BRIGHT + f"Sales Bot: {agent.messages[-1]['content']}")
while True:
    user_input = input(Fore.WHITE + Style.BRIGHT + "You: ")
    if user_input.lower() == "exit":
        print(Fore.GREEN + Style.BRIGHT + "Sales Bot: Goodbye!")
        break
    response = agent.invoke(user_input)
    print(Fore.GREEN + Style.BRIGHT + f"Sales Bot: {response}")